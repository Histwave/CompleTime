...
#include "ComplexityAnalysisPass.h" // 添加：复杂度分析
...
int CTPass::inlineFunctionCalls(Function &F,
  ...
  // if (num_functions_to_inline) {
  //   errs() << "{"
  //     << "\"function\": \"" << F.getName().drop_back(9) << "\", "
  //     << "\"inline_functions\": [";
  //   for (auto &F : functions_to_inline) {
  //     errs() << "\"" << F->getName() << "\", ";
  //   }
  //   errs() << "]}\n";
  // }
...
PassPluginLibraryInfo getPassPluginInfo() {
  const auto callback = [](PassBuilder &PB) {
    ...
    // 新增：ComplexityAnalysisPass 注册逻辑
    PB.registerOptimizerLastEPCallback(
#if LLVM_VERSION_MAJOR < 20
        [](ModulePassManager &MPM, OptimizationLevel Level) {
#else
        [](ModulePassManager &MPM, OptimizationLevel Level, ThinOrFullLTOPhase Phase) {
#endif
          MPM.addPass(llvm::ComplexityAnalysisPass());
          return true;
        });
